...
#[test]
fn markdown_image_fallback_explains_rejected_source() {
    let text = render_markdown_text("![diagram](http://example.org/diagram.png)");

    assert_eq!(
        plain_lines(&text),
        vec![
            "[image: diagram] (http://example.org/diagram.png) [image unavailable: unsupported image URL scheme: http]"
        ]
    );
}
